...
// ORDER BY mn.month_num, m.case_id;
    ...
    public static function get_cases_based_on_barangay_id($barangay_id){
        try{
            $sql = "SELECT 
                        ...
                        COUNT(m.case_id) AS case_count  -- This counts the number of cases in each purok
                    FROM 
                        barangay b
                    LEFT JOIN mental_health_case m ON m.barangay_id = b.barangay_id
                    LEFT JOIN purok p ON m.purok_id = p.purok_id
                    WHERE 
                        b.barangay_id = :barangay_id  -- Specific barangay_id you want to filter
                    GROUP BY 
                        b.barangay_id, b.barangay_name, p.purok_id, p.purok_name
                    ORDER BY
                        p.purok_name;  -- Optional: to order the result by purok name or other relevant column
                    ";
                    $stmt = self::conn()->prepare($sql);
